...
gltfLoader.load("./models.glb", (gltf) => {
  ...
  for (const position of positions) {
    ...
    for (let i = 0; i < particles.maxCount; i++) {
      ...
      if (i3 < originalArray.length) {
        ...
        const randowIndex = Math.floor(position.count * Math.random()) * 3;
        newArray[i3] = originalArray[randowIndex];
        newArray[i3 + 1] = originalArray[randowIndex + 1];
        newArray[i3 + 2] = originalArray[randowIndex + 2];
      }
    }
  ...
  //   particles.geometry.setIndex(null); // make particles not so bright
  particles.geometry.setAttribute(
    "aSize",
  ...
  particles.material = new THREE.ShaderMaterial({
    ...
    blending: THREE.AdditiveBlending, // set circle particles
    depthWrite: false, // to prevent the occluding issue
  });

  // Points
  particles.points = new THREE.Points(particles.geometry, particles.material);
  particles.points.frustumCulled = false; // turn off frustum culling
